...
class ResetPasswordVC: UIViewController, UITextFieldDelegate, NVActivityIndicatorViewable {
    ...
    @IBAction func actionSubmit(_ sender: Any) {
        ...
        print(param)
        self.startAnimating()
        RequestHandler.postRequest(url: Constants.URL.RESET_PASSWORD, params: param as NSDictionary, success: { (successResponse) in
            self.stopAnimating()
            let dictionary = successResponse as! [String: Any]
            self.showToast(message: dictionary["message"] as! String)
            
                let confirmationVC = self.storyboard?.instantiateViewController(withIdentifier: "LoginVC") as! LoginVC
